...
private:
    ...
    std::map<float, std::vector<std::vector<float>>> hashTable;
...
void PPF3DDetector::samplePCpoisson(std::vector<std::vector<float>> model, float sampleStep,
                                    std::vector<std::vector<float>> &sampledPoint) {
    printf("sampling pc ...\n");
    struct Index {
        int i;
        int j;
    ...
    int samplesInDimX = floor(b_box[1] / boxSize);
    int samplesInDimY = floor(b_box[2] / boxSize);
    int samplesInDimZ = floor(b_box[3] / boxSize);
    ...
    for (int count = 0; count < model.size(); count++) {
        ...
        if (count % 25000 == 0) {
            std::cout << "sample: " << std::setw(4) << round(100 * count / model.size()) << " %.\n";
        }
    }
        ...
        for (int j = 1; j <= samplesInDimY; j++) {
            for (int i = 1; i <= samplesInDimX; i++) {
                if (map[i][j][k] != 0) {
                    ...
                    float no = sqrt(model[count][3] * model[count][3] + model[count][4] * model[count][4] +
...
uint32_t hashPPF(std::vector<float> ppf_value, float angleRadians, float distanceStep) {
    const uint32_t key[4] = {int(ppf_value[0] / angleRadians), int(ppf_value[1] / angleRadians),
                             int(ppf_value[2] / angleRadians), int(ppf_value[3] / distanceStep)};
    int len = 16, seed = 42;
    ...
    for (int i = 0; i < refPointNum; i++) {
        ...
        for (int j = 0; j < refPointNum; j++) {
            if (i != j) {
                ...
                float dp = sampledPC[i][3] * sampledPC[j][3] + sampledPC[i][4] * sampledPC[j][4] +
                           sampledPC[i][5] * sampledPC[j][5];
        ...
        if (i % 80 == 0) {
            std::cout << "trained: " << std::setw(4) << round(100 * i / refPointNum) << " %.\n";
        }
    }
...
void Pose3D::updatePose(Pose3D &obj, Eigen::Matrix4f newPose) {
    ...
    pose_part << obj.pose(0, 0), obj.pose(0, 1), obj.pose(0, 2), obj.pose(1, 0), obj.pose(1, 1), obj.pose(1,
                                                                                                          2), obj.pose(
            2, 0), obj.pose(2, 1), obj.pose(2, 2);
...
void Pose3D::updatePoseT(Pose3D &obj, Eigen::Vector3f t) {
    Pose3D obj_temp;
    obj_temp.pose << obj.pose(0, 0), obj.pose(0, 1), obj.pose(0, 2), t(0), obj.pose(1, 0), obj.pose(1, 1), obj.pose(1,
                                                                                                                    2), t(
            1), obj.pose(2, 0), obj.pose(2, 1), obj.pose(2, 2), t(2), 0, 0, 0, 1;
...
void Pose3D::updatePoseQuat(Pose3D &obj, Eigen::Vector4f qNew) {
    ...
    Newpose << dcm(0, 0), dcm(0, 1), dcm(0, 2), oldPose(0, 3), dcm(1, 0), dcm(1, 1), dcm(1, 2), oldPose(1, 3), dcm(2,
                                                                                                                   0), dcm(
            2, 1), dcm(2, 2), oldPose(2, 3), 0, 0, 0, 1;
    obj.pose = Newpose;
...
bool comparePoses(Pose3D p1, Pose3D p2, float objectDiameter, float angleRadiansLoc) {
    float d = sqrt((p1.pose(0, 3) - p2.pose(0, 3)) * (p1.pose(0, 3) - p2.pose(0, 3)) +
                   (p1.pose(1, 3) - p2.pose(1, 3)) * (p1.pose(1, 3) - p2.pose(1, 3)) +
                   (p1.pose(2, 3) - p2.pose(2, 3)) * (p1.pose(2, 3) - p2.pose(2, 3)));
...
void PPF3DDetector::clusterPoses(std::vector<Pose3D> poseList, std::vector<std::vector<Pose3D>> &cluster,
...
void avg_quaternion_markley(Eigen::Vector4f &qs_avg, std::vector<Eigen::Vector4f> qs) {
    ...
    Eigen::Vector4f q;
    q << evecs.real()(0, evalsMax), evecs.real()(1, evalsMax), evecs.real()(2, evalsMax), evecs.real()(3,
                                                                                                       evalsMax);//得到对应特征向量
    qs_avg = q;
}

void
averageClusters(std::vector<std::vector<Pose3D>> cluster, std::vector<float> votes, std::vector<Pose3D> &resultPoses) {
...
std::vector<std::vector<float>> transformPose(std::vector<std::vector<float>> pc, Eigen::Matrix4f pose) {
    ...
    for (int count = 0; count < pc.size(); count++) {
        ...
        Eigen::Vector3f p_n;
        Eigen::Matrix3f p_M;
        p_M << pose(0, 0), pose(0, 1), pose(0, 2), pose(1, 0), pose(1, 1), pose(1, 2), pose(2, 0), pose(2, 1), pose(2,
                                                                                                                    2);
...
void PPF3DDetector::matchScene(std::vector<Pose3D> &result, bool saveVotersSwitch, bool recomputeScoreSwich,
                               std::vector<std::vector<float>> scene, float sceneFraction) {
    ...
    for (int count = 0; count < sceneSize;) {
        std::cout << "Matching:\t" << count + 1 << "\tof\t" << sceneSize << '\n';
        ...
        //存储位姿结果
        std::vector<std::vector<float>> *coordAccumulator = new std::vector<std::vector<float>>[(refNumLoc + 1) *
                                                                                                angleRelativeLoc];
        ...
        for (int j = 0; j < sizeof_accumulator; j++) {
            ...
            if (accumulator[j] > accuMax) {
                ...
                Eigen::Matrix4f iT;
                iT << iR(0, 0), iR(0, 1), iR(0, 2), it(0), iR(1, 0), iR(1, 1), iR(1, 2), it(1), iR(2, 0), iR(2, 1), iR(
                        2, 2), it(2), 0, 0, 0, 1;
                ...
                //Matrix按行压入，按列存储(0)-(n)。
                Eigen::Matrix4f TMax;
                TMax << Rmax(0, 0), Rmax(0, 1), Rmax(0, 2), tmax(0), Rmax(1, 0), Rmax(1, 1), Rmax(1, 2), tmax(1), Rmax(
                        2, 0), Rmax(2, 1), Rmax(2, 2), tmax(2), 0, 0, 0, 1;
...
void Writefile(std::string filename, std::vector<std::vector<float>> resPC) {
    ...
    for (int i = 0; i < resPC.size(); i++) {
        out << resPC[i][0] << '\t' << resPC[i][1] << '\t' << resPC[i][2] << '\t' << resPC[i][3] << '\t' << resPC[i][4]
            << '\t' << resPC[i][5] << '\n';
    }
    out.close();
}
